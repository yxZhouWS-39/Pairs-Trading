...
# 3. Use [Training Set] to Calculate Historical Indicators
...
# Get prices on the first day for initialization
first_day_ko = test_data['KO'].iloc[0]
first_day_pep = test_data['PEP'].iloc[0]
initial_capital = 10000.0

# 5.1 Calculate Daily Value for [Buy & Hold Benchmark]
bh_values = []
...
for m in threshold_multipliers:
    ...
    trade_status = 0 # 0: Base Position (50/50), 1: All-in KO (100/0), -1: All-in PEP (0/100)
    ...
    for date, row in test_data.iterrows():
        ...
        # Calculate current total assets (real net asset value fluctuating with stock prices at market open)
        current_val = cash + (ko_shares * ko_price) + (pep_shares * pep_price)
        portfolio_values.append(current_val)
        
        # Core All-in Rotation Logic
        if trade_status == 0:
            # 1. KO overvalued, PEP undervalued -> Sell all KO, go All-in on PEP
            if pct_dev >= current_threshold:
                ...
                trade_count += 1
            # 2. PEP overvalued, KO undervalued -> Sell all PEP, go All-in on KO
            ...
            # Under All-in PEP status, wait for deviation to revert to 0 axis -> Rebalance to 50/50 to start a new cycle
            if pct_dev <= 0:
            ...
            # Under All-in KO status, wait for deviation to revert to 0 axis -> Rebalance to 50/50 to start a new cycle
            if pct_dev >= 0:
                ko_shares = (current_val * 0.5) / ko_price
                pep_shares = (current_val * 0.5) / pep_price
                cash = current_val - (ko_shares * ko_price) - (pep_shares * pep_price)
    ...
    print(f"📈 Strategy {m:>.1f}σ Group | Final Value: ${portfolio_values[-1]:,.2f} | Total Return: {final_return:.2f}% | Trades Triggered: {trade_count} times")

# Print results for the Buy & Hold benchmark
bh_return = (bh_values[-1] - initial_capital) / initial_capital * 100
print(f"📊 Buy & Hold Benchmark | Final Value: ${bh_values[-1]:,.2f} | Total Return: {bh_return:.2f}%")

# ==========================================
# 6. Comprehensive Performance Plot
# ==========================================
